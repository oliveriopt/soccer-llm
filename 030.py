...
BEGIN
  ...
  order_ids_raw AS (
    ...
    FROM (
      ...
      JOIN params p ON TRUE
      WHERE ot.created_date >= p.start_dt OR ot.updated_date >= p.start_dt
...
Y me da este error:
Query error: Name created_date not found inside ot at [rxo-dataeng-datalake-np.sqlserver_to_bq_silver.sp_load_orders:71:16]

No quiero que la modificacion sea grande.

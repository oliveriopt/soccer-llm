...
END;
  ...
  estoy haciendo un code reviewy tengo estos comentarios:

Abhinav Chandna: "Are we building full loads and incremental builds similar to the source code?"

  y hago el llamado haciendo esto:

DECLARE p_lookback_days INT64 DEFAULT NULL;
CALL `sqlserver_to_bq_silver.sp_load_orders`(60);

  y tengo este comentario:
    
Abhinav Chandna: "Is 90 days something in the original code? and the first call that we are making should it be for 60 days?"

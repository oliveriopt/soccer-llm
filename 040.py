...
What you asked (changes):
- Read BOTH CSV formats:
    1) Semicolon-separated (;)
    2) Comma-separated (,)
  Auto-detect delimiter.
- Read BOTH JSON formats:
    1) JSON array (entire file is a list)
    2) JSONL (one JSON object per line)
- Analysis scope stays the same:
    QA totals/percentages computed ONLY over keys present on BOTH sides (common_keys).
...
SQLSERVER_CSV = BASE_DIR / "sqlserver_3.csv"   # can be ; or ,
...
BOOL_COLS_BQ = {"is_hazmat", "is_drop_and_hook"}
BOOL_COLS_SQL = {"IsHazmat", "IsDropAndHook"}
...
def sniff_csv_delimiter(path: Path, encoding: str) -> str:
    """
    Detect delimiter between ';' and ',' using csv.Sniffer.
    Fallback: if ';' appears more than ',' in first lines -> ';' else ','.
    """
    sample = path.read_text(encoding=encoding, errors="replace")[:8192]
    if not sample.strip():
        die(f"[SQL] CSV appears empty: {path}")
...
def load_sql_csv(path: Path) -> pd.DataFrame:
    ...
    for enc in encodings:
        try:
            delim = sniff_csv_delimiter(path, enc)
            df = pd.read_csv(
                path,
                sep=delim,
                dtype=str,
            ...
            # strip whitespace in column names (sometimes " OrderKey")
...
def compare(sql_df: pd.DataFrame, bq_rows: List[Dict[str, Any]]) -> CompareResult:
    ...
    # If duplicates exist on SQL side, keep last (same as dict overwrite)
    sql_by_key = {k: row for k, row in sql_df.set_index(SQL_KEY).iterrows() if str(k).strip()}
...
def main() -> None:
    ...
    if not bq_rows:
        die("[BQ] JSON/JSONL loaded but has 0 objects.")
    if SQL_KEY not in sql_df.columns:
        die(f"[SQL] Missing key column '{SQL_KEY}' in CSV. Found: {list(sql_df.columns)}")

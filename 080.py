Aquí va una versión de **stored procedure en BigQuery**, ya estructurada para mejor performance que tu query actual, con este comportamiento:

* `p_lookback_days IS NULL` → hace **full load lógico de 90 días**
* `p_lookback_days IS NOT NULL` → hace **incremental** usando el valor recibido

Además:

* materializa etapas pesadas en `TEMP TABLE`
* filtra temprano con `order_ids`
* evita los subqueries correlacionados para montos
* hace `MERGE` al target final

Ojo: **dejé los proyectos exactamente como aparecen en tu query** para que te sea más fácil comparar. Después conviene unificarlos a un solo ambiente.

```sql
CREATE OR REPLACE PROCEDURE `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.sp_load_orders`(
...
BEGIN
  ...
  WHERE ah.created_by = 'BazDataMigration'
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM `rxo-dataeng-datalake-np.sqlserver_to_bq_silver.baz_settled_order_migration` AS m2
  ...
  WITH accessorial AS (
    ...
    FROM `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.fn_order_total_accessorial_charge`()
    GROUP BY order_id
  )
  SELECT
    o.order_id,
  ...
  FROM order_ids o
  LEFT JOIN `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.fn_order_total_charge`() ch
    ON ch.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.fn_order_total_linehaul_charge`() lh
    ON lh.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.fn_order_total_fuel_charge`() f
    ON f.order_id = o.order_id
  LEFT JOIN accessorial a
    ON a.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-uat.sqlserver_to_bq_silver.fn_order_detention_charges`() d
    ON d.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-uat.sqlserver_to_bq_silver.fn_order_tonu_charges`() t
    ON t.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-uat.sqlserver_to_bq_silver.fn_order_total_linehaul_pay`() lhp
    ON lhp.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-uat.sqlserver_to_bq_silver.fn_order_total_fuel_pay`() fp
    ON fp.order_id = o.order_id
  LEFT JOIN `rxo-dataeng-datalake-uat.sqlserver_to_bq_silver.fn_order_total_pay`() cp
  ...
  FROM `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.xpomaster_orders_ordersalesrep` osr
  JOIN order_ids o
  ...
  LEFT JOIN `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.xpomaster_orders_sourcetype` AS src
  ...
  -------------------------------------------------------------------
  EXECUTE IMMEDIATE """
    CREATE TABLE IF NOT EXISTS `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.stagingbrokerage_staging_orders`
  ...
  -- 13) MERGE
  -------------------------------------------------------------------
  MERGE `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.stagingbrokerage_staging_orders` T
  USING tmp_src S
...
```sql
CALL `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.sp_load_orders`(NULL);
```

Incremental de 7 días:

```sql
CALL `rxo-dataeng-datalake-prd.sqlserver_to_bq_silver.sp_load_orders`(7);
```

Te digo lo más directo: esta versión ya está bastante mejor que tu query original, pero el siguiente salto real de performance sería **reemplazar el armado de `order_ids` incremental simple por un `order_ids_raw` unido desde todas las tablas que pueden cambiar el resultado**. Ahí sí quedarías mucho más cerca del comportamiento de SQL Server. Si quieres, en el próximo paso te la dejo en versión **v2**, con `order_ids_raw` multi-tabla.
